...
namespace F5IPConfigValidator
{
    ...
    class Processor
    {
                ...
                async Task ProcessIpString_(string ipString_)
                {
                    ...
                    foreach (var entry in addressSpaceIdMap)
                    {
                        try
                        {
                            ...
                            var records = await ValidateIpString(addressSpace, forestName, eopDcName, ipString_);

                            var ipRecord = records[0];
                            if (ipRecord.Status == ValidationStatus.NoMatch) continue;
                            if (ipRecord.Status == ValidationStatus.Unknown)
                            {
                            ...
                            if (ipRecord.Status != ValidationStatus.MultipleMatches &&
                                // Is cached success?
                                ipRecord != ValidationRecord.Success)
                            {
                                ...
                                if (!addressSpace.IsSameTextAs(mappedSpaceName))
                                {
                                    ...
                                    {
                                        AddressSpace = ipRecord.AddressSpace,
                                        Environment = ipRecord.Environment,
                                        Forest = ipRecord.Forest,
                                        EopDcName = ipRecord.EopDcName,
                                        Id = ipRecord.Id,
                                        Prefix = ipRecord.Prefix,
                                        IpamDcName = ipRecord.IpamDcName,
        ...
        private async Task<List<ValidationRecord>> ValidateIpString(
            ...
            string ipString)
        {
            var addressSpaceId = addressSpaceIdMap[addressSpace];
            ...
            {
                return new List<ValidationRecord> { ValidationRecord.NoMatch };
            }
            ...
            var validationRecords = new List<ValidationRecord>();
            var parent = queryResult[0];

            validationRecords.Add(ValidateAllocation_(parent));

            while (!string.IsNullOrEmpty(parent.ParentId))
            {
                parent = await IpamClient.GetAllocationAsync(addressSpaceId, parent.ParentId);
                var record = ValidateAllocation_(parent);
                record.Prefix = $"+{record.Prefix}";
                validationRecords.Add(record);
            }
            return validationRecords;

            ValidationRecord ValidateAllocation_(AllocationModel allocation_)
            {
                lock (prefixIdList)
                {
                    if (prefixIdList.Contains(allocation_.Id)) return ValidationRecord.Success;
                    prefixIdList.Add(allocation_.Id);
                }

                var prefix = allocation_.Prefix;
                allocation_.Tags.TryGetValue(SpecialTags.Title, out var title);
                allocation_.Tags.TryGetValue(SpecialTags.Datacenter, out var ipamDcName);
                allocation_.Tags.TryGetValue(SpecialTags.Region, out var region);
